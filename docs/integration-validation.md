...
后续进展：SDK 原生探针已获用户授权安装，项目 UID 已读通；构建 6 的局部选区对照仍返回全项目，选区读取未通过。详见 [SDK 记录](sdk-probe-validation.md)。下文保留第一轮实测范围。

## 当前产品范围更新

用户于 2026-09-12 决定暂不做选区，每次识别活动项目完整时间线。选区读取不再是当前版本验收门槛；下文选区负结果保留为历史证据。完整音频获取、剪辑时间映射与原时间线写回仍须实测。
