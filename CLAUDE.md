...
- Subloom 独立仓库、数据、安装与运行环境。VinciSub 仅只读参考，不改安装或用户数据；不复制 Resolve 宿主适配、落轨和 UI。
...
- 每次改动更新相关测试；交付前运行 `python3 -B -m unittest discover -s tests -v` 和相关真实探针。不能用单元测试取代 FCP 验收。
