...
最新界面：构建18（0.1.0）。本轮真实输入重检→1.7B识别→面板校对/苹方样式→关闭重开恢复→用户拖回→3条Title拆分→工程XML回读已通过。68项回归及原生编辑/身份测试通过；新拖放回调在修复后尚未再手动触发，连续自然30分钟素材仍待验收。以文末和 docs/MVP_PLAN.md 为最新状态，前面的构建8–17描述为历史。
