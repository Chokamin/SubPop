...
最新界面：构建17（0.1.0）已安装；双模型真实切换、30分钟项目处理、对白/音乐角色过滤、校对/样式、取消与恢复已实现。67项回归及真实后台验证通过；最终宿主新输入→校对恢复→Title落轨仍待用户拖入测试副本。以文末和 docs/MVP_PLAN.md 为最新状态，前面的构建8–15描述为历史。
