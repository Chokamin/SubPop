...
def resolve_model(model_id, root=ROOT):
    ...
    if spec.get('engine') == 'doubao':
        ...
        if not configured(root):raise ValueError('请在模型设置中配置语音 API Key')
        return None, None
...
def availability(root=ROOT):
    ...
    for spec in CATALOG['models']:
        if spec.get('engine') == 'doubao':
            ...
            result.append({'id': spec['id'], 'installed': ready, 'asrInstalled': False, 'hasFiles': False, 'reason': '' if ready else '尚需配置语音 API Key', 'sizeBytes': 0, 'cloud': True, 'cleanupPending': pending_count(root)})
            continue
        try:
