...
## 2026-09-12：构建23移除帮助/诊断按钮

用户要求去掉截图中的帮助和诊断。删除主面板两个按钮及其属性，词库/模型管理左移补齐空间；内部诊断记录保留，无新增隐藏入口。构建23使用26.5 SDK构建并安装、深度签名验证，86项回归通过。只重开空闲扩展与容器，没有重启识别worker或修改FCP项目。


## 2026-09-12：构建24迁移奇奇字幕整理规则与UI重排

用户要求整体查看奇奇字幕两频道学习后的优化并迁移，尤其默认无标点；同时重排美化UI和动画。只读检查VinciSub（HEAD3f448f0e0d4b5fd7af63c71490c440ed76190742）subtitles.py/text_units.py/worker.py与两份研究记录，参考为影视飓风/壹加壹25支19994条字幕统计+片段抽查，不是权重训练。未改参考项目安装、模型、词库或用户数据。

新增probes/editorial_rules.py/text_units.py独立纯规则，editorial.py适配SubPop逐词数据和有理数帧输出；run_job.finalize默认使用。先标点/停顿/中文词边界断句，再清理句子标点，保留3.5%、1/30、USB-C等；启用词库专名保护、数字单位、中英显示宽度（默认20）、<=0.2s接平、长停顿保护。固定jieba0.42.1安装到SubPop .venv（uv pip），锁文件更新。参考规则无法处理的远距离零时长块保留SubPop原断句并记录reviewWarnings，显示需校对数量，不伪造精准时间。原始ASR与历史已落轨内容不自动改写；未迁移参考稿/独立优化窗/字数设置。

同一22分钟1.7B旧结果复用，job a82907cc-3413-4cae-89d5-cedabbc49fcb得到616条（旧562），14块需校对；所有字母数字字符保留、常见中文句子标点为0、无重叠，数值技术标识保护单元测试通过。详细来源/适配边界docs/subtitle-rules-port.md，证据docs/evidence/subpop-editorial-ui.json。未重跑模型，不宣称准确率提高。

原生新增StudioChrome.h/StudioLayout.inc/StudioTests.m：深灰层次、紫色波形标识、顶部词库/模型、双列设置、分钟时长、自动整理说明、状态卡/真实进度、616行结果预览/样式/拖回。最大内容宽800、最小窗口580，消除宽窗大空隙与窄窗挤压。波形只在处理/下载时活动，状态/结果淡入、拖入反馈，尊重减少动态效果。模型列表卡片化，非下载时不显示旧满进度条；词库编辑区圆角。帮助/诊断不恢复。

构建24已安装/深度签名通过。93项独立环境测试无跳过全部通过；系统Python93项其中5项jieba集成跳过，独立环境已覆盖。原生616行三版本编辑/样式/时间harness与580/800/1100布局、进度/取消/波形开停测试通过，1.14DTD通过。FCP实际新版空闲页面截图检查通过；模型管理功能窗口检查通过（最后卡片微调后用户在FCP编辑，未继续抢前台复查）。用户此时已将旧562条拖入并拆开，实际FCP可见旧标点字幕，不能把616条新输出称已落轨。本轮没有任何FCP项目写入/共享。完成时新主面板可用、仍1.7B、词库0；内部诊断不再有UI入口。


## 2026-09-12：构建25优先显示拖回操作
