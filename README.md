...
| FireRedASR2-AED（本地候选） | CPU | 4.73 GB |

两个 Qwen 模型还需要约 **1.84 GB** 的共用时间对齐模型，首次下载会自动补齐，之后无需重复下载。处理速度取决于模型、项目长度和设备性能。

FireRedASR2-AED 使用模型自身的 CTC 时间对齐，无需下载 Qwen 对齐模型；当前采用 CPU 推理，速度需结合设备评估。对齐失败会提示重试，不用均分时间生成字幕。其推理代码来自 [FireRedTeam](https://github.com/FireRedTeam/FireRedASR2S)，采用 Apache-2.0 许可。
...
词库参与断句保护，并统一匹配词条的大小写、空格和等值数字写法。它不会把不同型号强行替换，也不能保证纠正所有同音词或错听。不同模型对识别提示词的支持不同；SenseVoice 和 FireRed 的词库在字幕整理阶段生效。
