...
| 普通字幕（默认） | 无底框，使用 FCP 自带基本字幕 |
| FCP 原生底框字幕 | 使用 FCP 自带自适应底框，无需 Tap5a |
| Tap5a 底框字幕 | 首次使用时可在 SubPop 内下载安装 **Tap5a Autosize Text Background** |

**Tap5a 不包含在 PKG 中，但可在 SubPop 内下载并安装。** 首次选择 Tap5a 样式时，点击「下载并安装 Tap5a」，授权一次「影片」文件夹，SubPop 会从作者源下载并放到正确目录。基础字幕无需 Tap5a。
...
5. **校对结果**：展开字幕预览，检查文字和断句，默认使用无底框的普通字幕，也可切换为 FCP 原生底框或 Tap5a 样式送回 FCP。
...
「普通字幕 · 无底框」是默认样式；需要底框时，可选择「FCP 原生 · 自适应底框」。两种都使用 FCP 内置模板。原生底框的背景、圆角和动画可在 FCP 检查器中调整。
