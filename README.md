...
1. 在[火山引擎豆包语音控制台](https://console.volcengine.com/speech/new/setting/apikeys)开通「大模型录音文件极速版」，并创建语音服务 API Key。此处使用语音服务密钥，不是方舟聊天模型密钥。
2. 在 SubPop 点击「模型」→「豆包云端识别」→「配置 API Key」，粘贴并保存。密钥保存在 macOS 钥匙串，保存时不会调用接口，也不验证额度。
3. 选择豆包模型，拖入项目并生成字幕。在确认音频上传和计费后，才开始云端任务。

仅音频上传至火山引擎，视频画面、FCP 项目文件和词库不上传。费用按你的火山引擎账户结算。长音频分段提交；取消、超时或网络中断后不会自动重试，已经提交的部分仍可能计费。返回结果沿用 SubPop 的本机词库规范化、断句和字幕样式流程。

当前已完成接口模拟与本机回归；**尚未使用已开通的真实账户验证云端识别效果和计费**。[官方接口说明](https://www.volcengine.com/docs/6561/1631584?lang=zh)
