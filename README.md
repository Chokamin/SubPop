...
2. 在 [TOS 控制台](https://console.volcengine.com/tos)创建**私有**存储桶，区域选择北京、上海或广州。SubPop 不会自动开通或创建付费资源。建议关闭桶版本控制，并为 `subpop-temp/` 前缀设置 1 天过期的生命周期规则，作为清理失败时的补充。
3. 创建仅可访问该桶 `subpop-temp/` 对象的子用户 AK/SK，授予上传、读取/查询对象、删除权限；开启版本控制的桶还需要读取和删除对象版本的权限。不要在聊天中发送凭证。
4. SubPop「模型」→「豆包录音文件识别 2.0」→「配置云端识别」填写语音 API Key、TOS 区域、桶名称及 AK/SK。密钥保存到 macOS 钥匙串；可以先只保存 API Key，之后补齐 TOS。保存不联网验证服务权限或额度。
5. 配置齐全后选择豆包模型，拖入项目，在确认音频上传和计费后开始识别。

2.0 的接口需要音频下载链接。SubPop 自动把约 5 分钟一段的音频上传到你的 TOS 桶，以 1 小时有效的签名链接提交给语音服务，然后查询结果。视频画面、FCP 项目文件和词库不上传；音频不会传到 SubPop 开发者的服务器。语音识别、TOS 存储与流量均可能按你的火山引擎账户计费。

正常完成、失败或取消时会尝试删除临时音频（开启版本控制时删除本次上传的具体版本）。断网、强制退出或缺少删除权限可能造成残留；本机只保留不含密钥、下载链接的清理记录，下次确认云端任务后先重试清理。设置中会显示待清理数量，也可在 TOS 控制台处理 `subpop-temp/`。切换桶或移除凭证不会替你清理旧桶，旧记录会保留提示。建议同时配置桶生命周期规则。

识别提交不自动重试；取消、超时或断网后，已经提交的部分仍可能计费。返回结果沿用 SubPop 的本机词库规范化、断句和字幕样式流程。

当前已完成接口模拟与本机回归；**尚无真实服务账户，TOS 权限、上传清理、云端识别效果与计费仍待联机验证**。[官方 2.0 接口说明](https://www.volcengine.com/docs/6561/1354868?lang=zh)
